...
import until.TestDBConnectionPool;

public class UpdateTestByJdbc830 {
	...
	public static String updatetSqData(Map<String, String> map) {
		...
		try {
			con = TestDBConnectionPool.getInstance().getConnection();
			System.out.println("TEST数据库连接成功2！");
			for (Map.Entry<String, String> entry : map.entrySet()) {
				String sql = "UPDATE("
						+ "select * from ("
						+ "select id,login_name, name ,enabled ,"
						+ "case when regexp_like(login_name,'([A-Z]+[0-9])') then '大写字母和数字' "
						+ "when regexp_like(login_name,'([A-Z])') then '大写字母'  "
						...
						+ ")where type_login_name='大写字母和数字'  or  type_login_name='大写字母' "
			...
			try {
				...
				if (con != null)
					con.close();
				System.out.println("TEST数据库连接已关闭！2");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static List<String> lookUpListEmp() {
		...
		try {
			...
			System.out.println("连接OFILM_HRDB数据库成功！1");
			...
			try {
				...
				if (con != null)
					con.close();
				System.out.println("OFILM_HRDB数据库连接已关闭！1");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static Map<String, String> dimissionEmp(List<String> lists) {
		...
		try {
			con = TestDBConnectionPool.getInstance().getConnection();
			System.out.println("连接Test数据库成功！1");
			...
			try {
				...
				if (con != null)
					con.close();
				System.out.println("TEST数据库连接已关闭！1");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
