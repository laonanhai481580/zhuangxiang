...
public class UpdateFpmByJdbc830 {
	...
	public static String updatetSqData(Map<String, String> map) {
		...
		try {
			...
			System.out.println("Fpm数据库连接成功2！");
			...
			try {
				...
				if (con != null)
					con.close();
				System.out.println("Fpm数据库连接已关闭！2");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static Map<String, String> dimissionEmp(List<String> lists) {
		...
		try {
			con = FpmDBConnectionPool.getInstance().getConnection();
			System.out.println("连接Fpm数据库成功！1");
			...
			try {
				...
				if (con != null)
					con.close();
				System.out.println("Fpm数据库连接已关闭！1");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
