...
public class UpdateSqByJdbc830 {
		...
		try {
			...
			for (Map.Entry<String, String> entry : map.entrySet()) {
				String sql = "UPDATE("
						...
						+ "and login_name<>'GX4 Sensor QC' and login_name<>'GX3 Sensor QC' "
			...
			try {
				...
				if (con != null)
					con.close();
				System.out.println("Sq数据库连接已关闭！2");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static Map<String, String> dimissionEmp(List<String> lists) {
		...
		try {
			con = SqDBConnectionPool.getInstance().getConnection();
			System.out.println("连接Sq数据库成功！1");
			String sql = "select * from ("
					...
					+ "when login_name is null then '空值' else '其他' end type_login_name "
					+ "from acs_user where enabled=1 "
					+ ")where type_login_name='大写字母和数字' and login_name<>'GX4 Sensor QC' and login_name<>'GX3 Sensor QC'";
			pre = con.prepareStatement(sql);
			result = pre.executeQuery();
			String str = "^[A-Z]{3}";
			while (result.next()) {
			...
			try {
				...
				if (con != null)
					con.close();
				System.out.println("Sq数据库连接已关闭！1");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
