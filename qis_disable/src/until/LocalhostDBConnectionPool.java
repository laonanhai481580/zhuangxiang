...
public class LocalhostDBConnectionPool {
	private static volatile LocalhostDBConnectionPool dbConnection;
	private ComboPooledDataSource cpds;

	/**
	 * 在构造函数初始化的时候获取数据连接
	 */
	private LocalhostDBConnectionPool() {
		try {
			/** 通过属性文件获取数据库连接的参数值 **/
	 ...
	 * 获取数据库连接对象，单例
	 */
	public static LocalhostDBConnectionPool getInstance() {
		if (dbConnection == null) {
			synchronized (LocalhostDBConnectionPool.class) {
				if (dbConnection == null) {
					dbConnection = new LocalhostDBConnectionPool();
				}
			}
		}
		return dbConnection;
